...
fn handle_watch(session_service: &SessionService) -> Result<(), Box<dyn Error>> {
    loop {
...
fn handle_generate_test_data(number: u32) -> Result<(), Box<dyn Error>> {
...
fn handle_command(cmd: Command, session_service: &SessionService) -> Result<(), Box<dyn Error>> {
    match cmd {
        ...
        Command::GenerateTestData { number } => {
            handle_generate_test_data(number)?;
        }
    }
    Ok(())
}

fn handle_tui(session_service: &SessionService) -> Result<(), Box<dyn Error>> {
...
fn handle_init_session_dir(session_service: &SessionService) -> Result<(), Box<dyn Error>> {
...
fn handle_active(session_service: &SessionService) -> Result<(), Box<dyn Error>> {
...
fn handle_find_sessions_in_range(
    session_service: &SessionService,
    start_date: String,
    end_date: String,
